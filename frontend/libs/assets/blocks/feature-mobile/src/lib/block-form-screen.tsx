...
export const BlockFormScreen = ({ navigation, route }: any) => {
  ...
  const { blocks, createBlock, updateBlock, isCreating, isUpdating } =
    useBlocks(orchardId);
  const { varieties, isLoading: isVarietiesLoading } = useVarieties();

  const { can } = usePermission();
  const canEdit = can(
    isEditing ? PERMISSIONS.BLOCK_EDIT : PERMISSIONS.BLOCK_CREATE
  );
  ...
  const [plantings, setPlantings] = useState<
    { varietyId: string; treeCount: string }[]
  ...
  const [currentPlantingIndex, setCurrentPlantingIndex] = useState<
    number | null
  >(null);
      ...
      if (block) {
        ...
        setPlantings(
          block.plantings.map((p) => ({
            varietyId:
              typeof p.varietyId === 'object'
                ? (p.varietyId as any)._id
                : p.varietyId,
  ...
  const validate = () => {
    ...
    const invalidPlanting = plantings.some(
      (p) => !p.varietyId || parseInt(p.treeCount) < 0
    );
    if (invalidPlanting)
      newErrors.plantings =
        'All plantings must have a variety and positive tree count';
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      plantings: plantings.map((p) => ({
        ...
        treeCount: parseInt(p.treeCount) || 0,
      })),
      ...(isEditing ? { isActive } : {}),
    };
    ...
    if (isEditing) {
      ...
      const hasChanged = plantings.some((p, i) => {
        ...
        const initialVarietyId = initial
          ? typeof initial.varietyId === 'object'
            ? (initial.varietyId as any)._id
            : initial.varietyId
          : null;
  ...
  const handleUpdatePlanting = (
    index: number,
    field: 'varietyId' | 'treeCount',
    value: string
  ) => {
    const newPlantings = [...plantings];
  ...
  return (
    ...
    >
      ...
      {plantings.map((planting, index) => (
        <View key={index} style={styles.plantingRow}>
          ...
          <TextInput
            ...
            onChangeText={(text) =>
              handleUpdatePlanting(index, 'treeCount', text)
            }
            mode="outlined"
          ...
          {!isView && plantings.length > 1 && (
            <IconButton
              icon="delete"
              onPress={() => handleRemovePlanting(index)}
            />
          )}
        </View>
      ))}
      ...
      {!isView && (
        <Button
          mode="outlined"
          onPress={handleAddPlanting}
          style={styles.addButton}
        >
          Add Planting
        </Button>
      )}
      {!isView && errors.plantings && (
        <HelperText type="error">{errors.plantings}</HelperText>
      )}
      ...
      <Portal>
        <Modal
          ...
          onDismiss={() => setVarietyModalVisible(false)}
