...
export const UserFormScreen = ({ navigation, route }: any) => {
  ...
  const canEdit = can(
    isEditing ? PERMISSIONS.USER_EDIT : PERMISSIONS.USER_CREATE
  );
  ...
  return (
    <FormLayout
      mode={mode}
      title={
        isEditing ? (isEditMode ? 'Edit User' : 'User Details') : 'New User'
      }
      onCancel={() => {
    ...
    >
      ...
      <SegmentedButtons
        value={userType}
        onValueChange={(val) =>
          handleChange(setUserType, val as UserType, 'userType')
        }
        buttons={[
      ...
      <View style={styles.chipContainer}>
        <Chip
          mode="outlined"
          onPress={() =>
            !isView && can(PERMISSIONS.ROLE_VIEW) && setRoleModalVisible(true)
          }
          style={styles.chip}
          icon="shield-account"
          onClose={
            !isView && roleId
              ? () => handleChange(setRoleId, undefined, 'roleId')
              : undefined
          }
      ...
      <View style={styles.chipContainer}>
        {clientIds.map((id) => {
          const client =
            ...
            availableClients.find((c) => c._id === id);
          return (
            <Chip
              key={id}
              onClose={!isView ? () => toggleClient(id) : undefined}
              style={styles.chip}
            >
      ...
      {isEditing && (
        <View style={styles.switchContainer}>
          ...
          <Switch
            value={isActive}
            onValueChange={(val) => handleChange(setIsActive, val, 'isActive')}
            disabled={isView}
          />
        </View>
      )}
