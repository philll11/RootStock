...
export const RoleFormScreen = ({ navigation, route }: any) => {
  ...
  const canEdit = can(
    isEditing ? PERMISSIONS.ROLE_EDIT : PERMISSIONS.ROLE_CREATE
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.response?.status === 409) {
        Alert.alert(
          'Conflict',
          'This role has been modified by another user. Please refresh and try again.'
        );
      } else {
  ...
  return (
    <FormLayout
      mode={mode}
      title={
        isEditing ? (isEditMode ? 'Edit Role' : 'Role Details') : 'New Role'
      }
      onCancel={() => {
    ...
    >
      ...
      <SegmentedButtons
        ...
        buttons={[
          ...
          {
            value: VisibilityScope.Subsidiary,
            label: 'Subsidiary',
            disabled: isView,
          },
